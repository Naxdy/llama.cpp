...
llama_model_glm_dsa::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    const bool is_mla = hparams.is_mla();
    GGML_ASSERT(is_mla);
    // DSA requires indexer tensors to be present
    if (model.layers[0].indexer_attn_q_b == nullptr) {
        throw std::runtime_error("GLM_DSA model is missing required indexer tensors");
    }
